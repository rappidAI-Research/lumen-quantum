...
def validate_special_token_order(sp: spm.SentencePieceProcessor, manifest: dict) -> dict[str, int]:
    ...
    for token, expected_id in EXPECTED_SPECIAL_TOKEN_IDS.items():
        if actual[token] != expected_id:
            raise ValueError(
                f"{token} muss Token-ID {expected_id} haben, hat aber {actual[token]}."
            )
    if list(
        manifest.get("expected_special_token_order", EXPECTED_SPECIAL_TOKEN_IDS.keys())
    ) != list(EXPECTED_SPECIAL_TOKEN_IDS.keys()):
        raise ValueError(
            "tokenizer_manifest.json dokumentiert nicht die stabile Special-Token-Reihenfolge."
        )
    for token, expected_id in EXPECTED_SPECIAL_TOKEN_IDS.items():
        manifest_id = int(manifest.get("token_ids", {}).get(token, -1))
        if manifest_id != expected_id:
            raise ValueError(
                f"Manifest-ID fuer {token} muss {expected_id} sein, ist aber {manifest_id}."
            )
    return actual
...
def validate_model_vocab_match(
    tokenizer_vocab_size: int, train_config_path: str | Path | None
) -> None:
...
def validate_final_tokenizer(
    ...
    missing = [
        filename for filename in REQUIRED_GGUF_FILES if not (tokenizer_dir / filename).exists()
    ]
    if missing:
        if allow_missing:
            return {
        ...
        raise FileNotFoundError(
            f"Finale Tokenizerdateien fehlen in {tokenizer_dir}: {', '.join(missing)}"
        )
    ...
    if actual_vocab_size != expected_vocab_size:
        raise ValueError(
            f"Tokenizer-vocab_size={actual_vocab_size}, erwartet {expected_vocab_size}."
        )
    if int(manifest.get("requested_vocab_size", -1)) != expected_vocab_size:
        raise ValueError(
            "tokenizer_manifest.json requested_vocab_size passt nicht zur finalen Config."
        )
    if int(manifest.get("future_llama_vocab_size", -1)) != expected_vocab_size:
        raise ValueError(
            "tokenizer_manifest.json future_llama_vocab_size passt nicht zur finalen Config."
        )
    ...
    report = {
        **base_report,
        ...
        "token_ids": token_ids,
        "gguf_files": {
            filename: sha256_file(tokenizer_dir / filename) for filename in REQUIRED_GGUF_FILES
        },
    }
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-freeze",
        action="store_true",
        help="Nur validieren, keinen FINAL_FROZEN Marker schreiben.",
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
        help="Nur Config/Pfade pruefen, fehlende Tokenizerdateien erlauben.",
    )
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if args.json:
        ...
        if report.get("final_validation", {}).get("config_only"):
            ...
            print(
                "Finaler Tokenizer validiert und eingefroren."
                if not args.no_freeze
                else "Finaler Tokenizer validiert."
            )
